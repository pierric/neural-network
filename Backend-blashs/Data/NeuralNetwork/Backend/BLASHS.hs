...
instance (InputLayer i, RealType p, SIMDable p, BodyTrans (ByBLASHS p) s)
       => Backend (ByBLASHS p) (i,s,SpecEvaluator) where
  type Env (ByBLASHS p) = Err
  type ComponentFromSpec (ByBLASHS p) (i,s,SpecEvaluator) o = SpecToCom (ByBLASHS p) s o
  type EvaluatorFromSpec (ByBLASHS p) (i,s,SpecEvaluator) o = Eval p
  compile b (i,s,o) opt = do c <- btrans b (isize i) s opt
                             case bwitness b s opt of
                               Dict -> return $ (c, Eval o)
  witness b (i,s,o) opt = let unM :: m a -> a
                              unM = undefined
                              c   = unM $ btrans b (isize i) s opt
                          in case bwitness b s opt of
                               Dict -> Dict
...
instance (Numeric p, RealType p, SIMDable p) => BodyTrans (ByBLASHS p) SpecFullConnect where
  ...
  type SpecToCom (ByBLASHS p) SpecFullConnect o = Stack (FullConn o p) (ActivateS p) CE
...
instance (Numeric p, RealType p, SIMDable p) => BodyTrans (ByBLASHS p) SpecConvolution where
  ...
  type SpecToCom (ByBLASHS p) SpecConvolution o = Stack (Convolute o p) (ActivateM p) CE
...
instance (Numeric p, RealType p, SIMDable p) => BodyTrans (ByBLASHS p) SpecMaxPooling where
  ...
  type SpecToCom (ByBLASHS p) SpecMaxPooling o = MaxPool p
...
instance (Numeric p, RealType p, SIMDable p) => BodyTrans (ByBLASHS p) SpecReshape2DAs1D where
  ...
  type SpecToCom (ByBLASHS p) SpecReshape2DAs1D o = Reshape2DAs1D p
...
instance (Numeric p, RealType p, SIMDable p) => BodyTrans (ByBLASHS p) SpecLSTM where
  ...
  type SpecToCom (ByBLASHS p) SpecLSTM o = Stack (LSTM o p) (ActivateS p) (LiftRun (Run (LSTM o p)) (Run (ActivateS p)))
...
instance (BodyTrans (ByBLASHS p) a) => BodyTrans (ByBLASHS p) (SpecFlow a) where
  --
  type SpecToCom (ByBLASHS p) (SpecFlow a) o = Stream o (SpecToCom (ByBLASHS p) a o)
