...
module Data.Tensor.SIMD (
  ...
  selectVector,
  SIMDable(..),
  squared_error', relu, relu', tanh, tanh'
) where
...
squared_error' :: SIMDable a => SIMDPACK a -> SIMDPACK a -> SIMDPACK a
squared_error' a y = selectVector (compareVector GE a y)
