...
module Data.Tensor.Execute (
  Statement(..), Order(..), Transpose(..), E.TensorWrap(..), E.DimWrap(..), Var(..),
  VarWithDim, _vid_VarWithDim,
  isAlloc, isBind, isStore, isCopy, isGEMV, isGERU, isGEMM, isDotAdd, isDotMul, isDotSca, isBindToTensor, isStoreTo, isDotAddTo, isDotScaTo,
  substitute,
  CGState, CG, runCG, initCG, toStatements, execute, execute',
) where
...
import Text.Printf
import Text.PrettyPrint.Free (Pretty(..), Doc, fill, text, parens, hsep, vcat, (<+>))
...
instance Show a => Pretty (Statement a) where
  pretty (Alloc d v) = fill 8 (text "Alloc") <+> parens (text (show d))  <+> text (show v)
  pretty (Bind  v t) = fill 8 (text "Bind")  <+> text (show v)  <+> text (show t)
  pretty (Store v t) = fill 8 (text "Store") <+> text (show v)  <+> text (show t)
  pretty (Copy v1 v2)= fill 8 (text "Copy")  <+> text (show v1) <+> text (show v2)
  pretty (BlasGEMV o t v1 v2 a b v3)     = fill 8 (text "BlasGEMV") <+> hsep (map text [show o, show t, show v1, show v2, show a, show b, show v3])
  pretty (BlasGERU o v1 v2 a v3)         = fill 8 (text "BlasGERU") <+> hsep (map text [show o, show v1, show v2, show a, show v3])
  pretty (BlasGEMM o t1 v1 t2 v2 a b v3) = fill 8 (text "BlasGEMM") <+> hsep (map text [show o, show t1, show v1, show t2, show v2, show a, show b, show v3])
  pretty (DotAdd v1 v2 v3)               = fill 8 (text "DotAdd")   <+> hsep (map text [show v1, show v2, show v3])
  pretty (DotMul v1 v2 v3)               = fill 8 (text "DotMul")   <+> hsep (map text [show v1, show v2, show v3])
  pretty (DotSca a  v1 v2)               = fill 8 (text "DotSca")   <+> hsep (map text [show a , show v1, show v2])
...
execute :: Element a => [Statement a] -> IO ()
execute ss = do 
  r <- evalStateT (runExceptT $ eval ss) M.empty
  case r of
    Left e  -> throwError $ userError $ show e
    Right _ -> return ()

execute' :: Element a => [Statement a] -> EvalS a -> IO (EvalS a)
execute' ss st = do
  (r, s) <- runStateT (runExceptT $ eval ss) st
  case r of
    Left e  -> throwError $ userError $ show e
    Right _ -> return s
